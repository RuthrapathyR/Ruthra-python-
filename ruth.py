        ...
        def submit1():
            ...
            if inp_qty11=="" or inp_rate11=='' or inp_id11=='' or inp_name11=='' or inp_cat11=='':
                ...
                if inp_qty11.isalpha() == True:
                    messagebox.showwarning('Warning!', "Entered quantity is wrong! Only numbers allowed!")
                    a = False
            if a!=False:
...
#secon pro namee
        def q_p():
            ...
            p1.resizable(0, 0)
            product_id=Label(p1,text='Product Id',font='30',bg=input_color).grid(row=1,column=0,pady=30,padx=15)
            quantity_purchased=Label(p1,text='Quantity in Stock',font='30',bg=input_color).grid(row=2,column=0,padx=15)
            s1 = '''select product_id from product ;'''.format()
            ...
            def submit111():
                ...
                s = '''update product set Quantity_in_stock='{}' where product_id={};'''.format(quantity_purchased11 ,(pro_id111) )
                o = [s]
                sta=True
...
#product_rate
        def p_r():
            ...
            p11.resizable(0, 0)
            product_id1=Label(p11,text='Product Id',font='30',bg=input_color).grid(row=1,column=0,pady=30,padx=15)
            product_rate=Label(p11,text='Rate',font='30',bg=input_color).grid(row=2,column=0,padx=15)
            s11 = '''select product_id from product ;'''.format()
            ...
            def submit1111():
                ...
                s = '''update product set Rate='{}' where product_id={};'''.format(product_rate11 ,(pro_id1111) )
                o = [s]
                sta=True
                if product_rate11 == "" :
                    sta=False
    ...
    def delete_product():
        ...
        pa1.protocol("WM_DELETE_WINDOW", on_closing1p5)
        w = 350
        h = 130
    ...
    def search():
        ...
        product_ide = Label(search_tk, text='Product Id :', font='40', bg=input_color).grid(row=0, column=0, pady=30,
                                                                                            padx=15)
        dv = '''select product_id from product ;'''.format()
        ...
        def submit5678():
            ...
            if pro1111_id2 == '':
                ...
                inp_rate12 = Label(search1_tk, text="  PRODUCT_RATE           :", bg=input_color, font='bold 15', pady=20,
                                   justify='left').grid(
                    row=5, column=0, sticky='w')
                s = '''select * from product where product_id = {};'''.format((pro1111_id2))
